...
public class QueryExecuter
{
	...
	public void executeQuery (JSONObject jObj, JSONObject toReturn, JSONArray err) throws Exception
	{

		//Vector<Input> readableFiles = new Vector<Input> ();
  	
			//Request r = new Request (, exe);
			int wanted = 0;
	    	...
	    	if (o == null)
					throw new IllegalArgumentException ("don't understand option: " + jArr.get (i));
	    	wanted |= o.value;
			}
			...
			if (wanted < 1)
				throw new IllegalArgumentException ("nothing to do. (no options provided?)");
			...
			if (files.size () == 1)
				...
				exe.executeCompare (files.firstElement (), files.get (1), toReturn, wanted);
	...
	public final void printUsage (String msg, PrintWriter out)
	{
		...
		out.println ("<html><head><title>BiVeS WebService USAGE</title></head><body>");
		out.println ("<style>p{max-width:50em;}	pre{font-size:.9em;background-color: #ddd;padding: 20px;}	</style>");
		out.println ("<h1>BiVeS WebService USAGE</h1>");
		out.println ("<p>To use this web service send a JSON object via post request.");
		...
		out.println ("</pre>");
		out.println ();
		out.println ("<p>files is an array of max. two files, either defined by plain XML or URLs to retrieve the files.</p>");
		out.println ();
		out.println ();
		...
		for (String key : keys)
			out.println ("\t"+key + Tools.repeat (" ", longest - key.length ()) + options.get (key).description);
		out.println ();

		out.println ("ADDITIONAL OPTIONS for single files");
		for (String key : addKeys)
			out.println ("\t"+key + Tools.repeat (" ", longest - key.length ()) + addOptions.get (key).description);
		out.println ("</pre>");
		out.println ();
		out.println ();
		
		out.println ("<p>an example call to compute the diff between two SBML file");
		out.println ("and ask for the highlighted chemical reaction network encoded");
		out.println ("in DOT language (crnDot) and the report encoded in HTML (reportHtml)");
		...
		out.println ("\t\"files\":");
		out.println ("\t[");
		out.println ("\t\t\"http://budhat.sems.uni-rostock.de/download?downloadModel=24\",");
		out.println ("\t\t\"http://budhat.sems.uni-rostock.de/download?downloadModel=25\"");
		out.println ("\t],");
		out.println ("\t\"options\":");
		out.println ("\t[");
		out.println ("\t\t\"SBML\",");
